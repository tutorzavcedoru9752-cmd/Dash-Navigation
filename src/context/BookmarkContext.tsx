...
import { getOrCreateDeviceId, saveDeviceId, generateNewDeviceId } from '../utils/deviceId';
...
interface BookmarkContextType {
  ...
  userIp: string | null;
  deviceId: string;
  switchDeviceId: (newId: string) => Promise<boolean>;
  resetDeviceId: () => void;
  isLoading: boolean;
}
...
export function BookmarkProvider({ children }: { children: ReactNode }) {
  ...
  const syncToBackend = async (newCategories: Category[], currentDeviceId: string) => {
    try {
  ...
  const loadDataForDevice = async (targetDeviceId: string) => {
    setIsLoading(true);
    try {
      const res = await fetch(`/api/categories?deviceId=${encodeURIComponent(targetDeviceId)}`, {
        headers: {
          'x-device-id': targetDeviceId
        }
      });
      ...
      if (data.categories && Array.isArray(data.categories) && data.categories.length > 0) {
        ...
        localStorage.setItem(`dash_categories_${targetDeviceId}`, JSON.stringify(data.categories));
        localStorage.setItem('dash_categories', JSON.stringify(data.categories));
      } else {
        // Fallback to local storage
        const saved = localStorage.getItem(`dash_categories_${targetDeviceId}`) || localStorage.getItem('dash_categories');
        if (saved) {
          try {
            const parsed = JSON.parse(saved);
            setCategories(parsed);
            if (!data.error) syncToBackend(parsed, targetDeviceId);
          } catch (e) {
      ...
      console.warn("Failed to fetch initial categories from backend, using local cache:", err);
      const saved = localStorage.getItem(`dash_categories_${targetDeviceId}`) || localStorage.getItem('dash_categories');
      if (saved) {
        try {
  ...
  useEffect(() => {
    if (hasInitialized && !isLoading) {
      ...
      localStorage.setItem(`dash_categories_${deviceId}`, JSON.stringify(categories));
      syncToBackend(categories, deviceId);
    }
  }, [categories, hasInitialized, isLoading, deviceId]);

  const switchDeviceId = async (newId: string): Promise<boolean> => {
    const trimmed = newId.trim();
    if (!trimmed) return false;
    saveDeviceId(trimmed);
    setDeviceId(trimmed);
    return await loadDataForDevice(trimmed);
  };
